...
class Product(Base):
    ...
    alternative_markings: Mapped[str | None] = mapped_column(Text, nullable=True)
    manufacturing_country: Mapped[str | None] = mapped_column(String(120), nullable=True)
    ...
    description: Mapped[str | None] = mapped_column(Text, nullable=True)
    require_prepayment_override: Mapped[bool | None] = mapped_column(Boolean, nullable=True)
    cod_allowed_override: Mapped[bool | None] = mapped_column(Boolean, nullable=True)
    ...
    bearing_spec: Mapped["BearingSpec | None"] = relationship(
        back_populates="product", uselist=False, cascade="all, delete-orphan"
    )
    belt_spec: Mapped["BeltSpec | None"] = relationship(
        back_populates="product", uselist=False, cascade="all, delete-orphan"
    )
    seal_spec: Mapped["SealSpec | None"] = relationship(
        back_populates="product", uselist=False, cascade="all, delete-orphan"
    )
    lubricant_spec: Mapped["LubricantSpec | None"] = relationship(
        back_populates="product", uselist=False, cascade="all, delete-orphan"
    )
    slug_redirects: Mapped[list["ProductSlugRedirect"]] = relationship(
...
class ProductSlugRedirect(Base):
    ...
    old_slug: Mapped[str] = mapped_column(String(220), unique=True, index=True, nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, nullable=False)
    product: Mapped["Product"] = relationship(back_populates="slug_redirects")
...
class Order(Base):
    ...
    stock_state: Mapped[str] = mapped_column(String(24), default="committed", index=True, nullable=False)
    stock_reserved_until: Mapped[datetime | None] = mapped_column(DateTime, index=True, nullable=True)
